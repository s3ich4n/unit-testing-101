...
- bad case
    ...
    - 단일 작업을 수행하는 데 여러 메소드를 호출해야 한다는 점 → 불변 위반(_invariant violation_). 캡슐화를 깨는 것을 의미한다. 캡슐화를 깨지 않도록 코드를 작성할 것!
...
- 엄격한 명명정책보다는 쉬운 이름으로. 복잡한 동작은 코드로 설명되도록.
...
이쯤되면 사실을 단도직입적으로 썼다할 수 있다. 이 테스트 케이스는, 테스트 대상의 애플리케이션 동작의 관점 중 하나를 설명한다. "배송가능" 여부는 현재 이후의 날짜여야 한다는 점이다.
